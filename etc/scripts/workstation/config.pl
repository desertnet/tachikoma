...
var home     = `echo ~/.tachikoma/services`;
...
cd server_log:ruleset:config
  add  100 deny where payload=.* FROM: .* ID: "tachikoma\@nyx|tachikoma\@misa" COMMAND: .*
...
cd local_system_log:ruleset:config
  add 100 allow where payload="(nyx|misa).* sudo"
  add 1000 deny
cd ..
...
cd topic_top
  ...
  cd clients
    ...
    listen_inet 0.0.0.0:4391
    connect_node 0.0.0.0:4391 topic_top:tee
  cd ..
  secure 3
cd ..
...
cd http
  ...
  make_node HTTP_Auth         tools:auth /Users/chris/Sites/tools/.htpasswd misa-tools
  make_node HTTP_Route        tools
  make_node HTTP_File         root:dir   /Users/chris/Sites
  make_node HTTP_File         tools:dir  /Users/chris/Sites/tools /tools
  make_node JobFarmer         CGI        4 CGI /Users/chris/Documents/desert.net/Projects/tachikoma/trunk/etc/Devel/CGI.conf /tmp/http
  command CGI   autokill on
  command CGI   lazy on
  command root  add_path /              root:dir
  command root  add_path /tools         tools:auth
  command root  add_path /cgi-bin       CGI
  command tools add_path /              tools:dir
  command tools add_path /debug/capture http:log
  connect_node tools:auth   tools
  connect_node http:log     _parent/http_log
  connect_node responder    root
  connect_sink 0.0.0.0:4242 responder

  # listen_inet --io 0.0.0.0:4243
  ...
  # connect_sink 0.0.0.0:4243    proxy:responder
  secure 3
cd ..
