...
is( ref $shell, 'Tachikoma::Nodes::Shell3', 'Shell3->new is ok' );

sub parse {
    my $input = shift;
    # Use the new parsing pipeline
    my $tokens = $shell->tokenize($input);
    # print STDERR Dumper($tokens);
    my $ast = $shell->build_ast($tokens);
...
is( $answer, "[hello][]\n", 'execute_ast_node sends commands' );

#####################################################################

$parse_tree = &parse('var foo=5');
is_deeply(
    $parse_tree,
    {
        'commands' => [
...
is_deeply( $var->{foo}, 5,  'var builtin sets variables correctly' );

#####################################################################

$parse_tree = &parse('var bar=<foo> + 5');
$answer     = q();
$shell->execute_ast_node($parse_tree);
is( $answer,     "", 'nothing is sent by var builtin' );
is( $var->{bar}, 10, 'arithmetic without parenthesis is evaluated' );

#####################################################################

$parse_tree = &parse('bar=<foo> + 3');
$answer     = q();
$shell->execute_ast_node($parse_tree);
...
$parse_tree = &parse('var baz ||= (<foo> + 4); baz //= (<foo> + 11)');
$answer     = q();
$shell->execute_ast_node($parse_tree);
...
$parse_tree = &parse('date; bar = <foo> + 7');
$answer     = q();
$shell->execute_ast_node($parse_tree);
...
$parse_tree = &parse('send echo (2 - -1)');
$answer     = q();
$shell->execute_ast_node($parse_tree);
$parse_tree = &parse('send echo (-2 + 1)');
$shell->execute_ast_node($parse_tree);
...
$parse_tree = &parse('{ send echo foo ; date }');
$answer     = q();
$shell->execute_ast_node($parse_tree);
is( $answer,
    "{foo }\n[date][]\n",
...
$parse_tree = &parse('send echo hi // there\!');
$answer     = q();
$shell->execute_ast_node($parse_tree);
...
$parse_tree = &parse("send echo 'foo'\\''bar'");
$answer     = q();
$shell->execute_ast_node($parse_tree);
...
$parse_tree = &parse('send echo "\<test\>"');
$answer     = q();
$shell->execute_ast_node($parse_tree);
is( $answer, "{<test>}\n",
...
$parse_tree = &parse('send echo   foo --set arg=\"one two\"  ');
$answer     = q();
$shell->execute_ast_node($parse_tree);
is( $answer,
...
$parse_tree = &parse('for x (((1)+1)) { send echo <x> }');
$answer     = q();
$shell->execute_ast_node($parse_tree);
is( $answer, "{2 }\n",
...
$parse_tree = &parse('for x ((1 + (0 || 1))) { send echo <x>; }');
$answer     = q();
$shell->execute_ast_node($parse_tree);
is( $answer, "{2}\n",
...
$parse_tree = &parse('if ( 0 == (1 && 0) ) { send echo ok; }');
$answer     = q();
$shell->execute_ast_node($parse_tree);
...
$parse_tree = &parse('send echo ( ( 1 * 3 ) + 5 )');
$answer     = q();
$shell->execute_ast_node($parse_tree);
...
$parse_tree = &parse('send echo if foo var eval func return okay');
$answer     = q();
$shell->execute_ast_node($parse_tree);
is( $answer,
...
$parse_tree = &parse('if ("foo eq bar" eq "foo eq bar") { version; }');
$answer     = q();
$shell->execute_ast_node($parse_tree);
$parse_tree = &parse('if ("foo eq bar" ne "foo eq bar") { version; }');
$shell->execute_ast_node($parse_tree);
is( $answer, "[version][]\n", 'quoted operators are not evaluated' );

#####################################################################

$parse_tree = &parse('if ("" eq "foo") { version; }');
$answer     = q();
$shell->execute_ast_node($parse_tree);
...
$parse_tree = &parse('if (1 < {date;}) { send echo yes; }');
$answer     = q();
$shell->execute_ast_node($parse_tree);
is( $answer, "[date][]\n{yes}\n",
...
$parse_tree = &parse('if ({date} > 1) { send echo yes; }');
$answer     = q();
$shell->execute_ast_node($parse_tree);
is( $answer, "[date][]\n{yes}\n",
...
$parse_tree = &parse('send echo ((foo eq foo)"\n");');
$answer     = q();
$shell->execute_ast_node($parse_tree);
...
is( $answer,
    "{one  two\n}\n",
    'variables with list values normalize whitespace'
);
