...
sub fill {
    ...
    return
        if ( ( not $type & TM_BYTESTREAM and not $type & TM_EOF )
        or $message->[FROM] eq '_parent' );
    return $self->expire_filehandles if ( $message->from eq 'Timer' );
    my $prefix = $self->{arguments} or die "ERROR: no prefix specified";
    my $stream = $message->[STREAM] or die "ERROR: no stream specified";
    my $op     = undef;
    my $relative = undef;
