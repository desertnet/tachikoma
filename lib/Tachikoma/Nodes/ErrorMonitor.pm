...
sub fill {
    ...
    my $email_thresholds = $self->{email_thresholds};
    my $trap_thresholds  = $self->{trap_thresholds};
    # XXX:
    ...;
...
sub send_alerts {
    ...
    if ( $type eq 'email' ) {
        ...
        local $ENV{PATH} = q();
        my $subject = 'WARNING: ErrorMonitor threshold(s) exceeded';
        ...
        if ( $self->{trap} ) {
            $message->[PAYLOAD] = join q(),
                "ErrorMonitor threshold(s) exceeded\n",
                $self->{trap};
        }
        else {
