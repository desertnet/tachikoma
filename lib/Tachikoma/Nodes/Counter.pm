...
sub new {
    ...
    my ( $sec, $min, $hour, $day ) = localtime($Tachikoma::Now);
...
sub arguments {
    ...
    if (@_) {
        ...
        die "invalid count mode"
            if ( $count_mode and not grep $count_mode eq $_,
            qw( simple sizes payloads ) );
        die "invalid send mode"
            if ( $send_mode and not grep $send_mode eq $_,
            @Windows, 'cancel', 'payloads' );
...
sub fire {
    ...
    push( @$one_minute,     $second );
    push( @$five_minute,    $second );
    push( @$fifteen_minute, $second );
...
$C{averages} = sub {
    ...
    my $calculate = sub {
        my $this     = shift;
        my $count    = shift;
        my $last     = join( '', 'last_', $this );
        my @averages = ( @{ $patron->{$last} }, @{ $patron->{$this} } );
    ...
    if ( $command->arguments ne '-s' ) {
        ...
        $response = join( '',
            'one_min_avg:',     $one_minute_avg,     ' ',
            'five_min_avg:',    $five_minute_avg,    ' ',
            'fifteen_min_avg:', $fifteen_minute_avg, ' ',
            'hour_avg:',        $hour_avg,           ' ',
            'day_avg:',         $day_avg,            "\n" );
    }
...
$C{set_count_mode} = sub {
    ...
    die "invalid count mode"
        if ( not grep $arguments eq $_, qw( simple sizes payloads ) );
...
$C{set_send_mode} = sub {
    ...
    my $arguments = $command->arguments;
    die "invalid send mode"
        if ( not grep $arguments eq $_, @Windows, 'payloads' );
...
sub calculate {
    ...
    my $count    = shift;
    my $last     = join( '', 'last_', $this );
    my @averages = ( @{ $self->{$last} }, @{ $self->{$this} } );
