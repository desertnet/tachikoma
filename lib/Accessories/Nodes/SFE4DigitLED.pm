...
sub fire {
    ...
    if ( $self->hashes_differ( $last, $pend ) ) {
        ...
        $self->verbose("chunk valid until $pend->{expire}")
            if ( $pend->{expire} );
        $self->send( $self->format_chunk($pend) );
        $self->send( $self->brightness( $pend->{bright} ) )
            if ( $pend->{bright} );
    }
    else {
...
sub hashes_differ {
    ...
    my $b    = shift;
    my $getv = sub {
        [   sort grep ref || s(^\d)(_)o || 1,
            grep defined,
            @{ [ values %{ $_[0] } ] }
        ]
    };
    return 0 if ( $a ~~ $b and &$getv($a) ~~ &$getv($b) );
    return 1;
}
